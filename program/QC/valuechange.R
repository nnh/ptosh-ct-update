...
df_used <- before_match %>% select(Codelist_Code, Code, temp_used)
...
submission_value_change$used <- submission_value_change$temp_used
submission_value_change <- submission_value_change %>% select(-c(temp_used, temp_seq))
